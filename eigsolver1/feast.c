...
{
    int myN_loc = info.N_loc[info.myid];
    ...
    int i, j, k;
    int length = N * M0;
    //printf("aaaaaaaaaaaaaaaaaaaaaaaaa  myid %d \n", info.myid);
    if (info.myid == 0)
    {
        ...
        for (i = 1; i < info.np; i++)
        {
            ...
            MPI_Send(sendbuf, nrow * M0, MPI_DOUBLE, i, i, MPI_COMM_WORLD);
            offset = offset + nrow;
        }
        free(sendbuf);
        free(Q);
    ...
    {
        MPI_Status status;
        MPI_Recv(Q_loc, myN_loc * M0, MPI_DOUBLE,  0, info.myid, MPI_COMM_WORLD, &status);
    }
    //printf("bbbbbbbbbbbbbbbbbbbbbbbbbbb  myid %d \n", info.myid);
    ...
    if (info.mumpsid == 0)
    {
        ...
        for (i = 0; i < info.np; i++)
        {
            if (i < info.nGau)
            {
                if (i < info.myid)
                {
                    MPI_Recv(WY,  myN_loc * M0, MPI_DOUBLE, i, mpitag, MPI_COMM_WORLD, &status);
                    ...
                    MPI_Send(sendbuf, sendlength, MPI_DOUBLE, i, mpitag, MPI_COMM_WORLD);
                    // if (i == 2)
...
void Feast_Project(double *Q_loc, double *lambda, double *Y_loc, SPMultiply SPMul, Info info)
{
    ...
    int M0 = info.M0;
    int i, j;
    int N_loc = info.N_loc[info.myid];
...
void Feast_GetRes(double *maxres, double *lambda, double *Q_loc, SPMultiply SPMul, Info info)
{
    ...
    if (info.myid == 0)
    {
        ...
        *maxres = 0;
        ...
        printf("\t\t maxres: %lf  neig %d %d \n", *maxres, neig, info.neig);

        if (neig > info.neig)
        {
            for (i = 0; i < neig - info.neig; i++)
            {
                res[maxid] = 0;
                lambda[maxid] = Emax + 1;
...
int  Feast_GetNumber(double *Y_loc, Info info)
{
    ...
    MPI_Bcast(&neig, 1, MPI_INT, 0, MPI_COMM_WORLD);
    free(Bq);
    free(Bq_loc);
    free(work);
    free(lambda_B);
...
获取估计的特征值数目
*****************************************/
void Feast_OrthAndGetNumber(int *neig, double *Q_loc, double *Y_loc, Info info)
{
    ...
    if (info.myid == 0)
    {
        ...
        dsyev_(&char_V, &char_U, &M0, Bq_loc, &M0, lambda_B, work, &length, &error);//Bq * Bq' = lambda * Bq'
        if (error != 0)
            printf("error in dsyev_ error %d Mo  %d\n", error, M0);
    ...
    MPI_Bcast(Bq_loc, M0 * M0, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    dgemm_(&char_N, &char_N, &N_loc, &M0, &M0, &one, Y_loc, &N_loc, Bq_loc, &M0, &zero, Q_loc, &N_loc); //Q = Y
