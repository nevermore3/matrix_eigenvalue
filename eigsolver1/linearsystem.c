...
//printf("jsjsj\n");
    if (info.myid == 0)
    {
        ...
        for (i = 1; i < info.np; i++)
        {
            ...
            MPI_Recv(recvbuf, nrow * M0, MPI_DOUBLE,  i, i,MPI_COMM_WORLD, &status);
    ...
    {
        MPI_Send(Q_loc,   myN_loc * M0, MPI_DOUBLE, 0, info.myid, MPI_COMM_WORLD);
    }
    int length = N * M0;
//printf("jsjsjsjsj\n");
    if (info.mumpsid == 0)
    {
        //MPI_Bcast(Q, length, MPI_DOUBLE, 0, info.hostcomm);
        my_MPI_Bcast(Q, length, info.hostcomm);
        ...
        while (length > 0)
        {
            ...
            my_MPI_Bcast(Q + myoffset, mylength, info.hostcomm);
...
void LS_InitLS(LinearSystem *LSystem, Info info)
{
    ...
    LS_SetGauss(info.nGau, info.hostid, &xe, &we);
    theta = ab * xe + ab;
