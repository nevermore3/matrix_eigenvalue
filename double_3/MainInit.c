...
void InitInfo(Info *info)
{
    ...
    int ninterval = 1;
    double mineig = 0.0;
    double propotion = 0.1;

    (*info).nGau = nGau;
    (*info).ninterval = ninterval;
    (*info).mineig = mineig;
    (*info).propotion = propotion;

    (*info).scal = calloc(1, sizeof(double));
    (*info).scal[0] = 7;
    (*info).interval = calloc(2, sizeof(double));
    (*info).history = calloc(2, sizeof(int));
    (*info).modifyregion = calloc(2, sizeof(double));
    (*info).neig = calloc(ninterval, sizeof(int));
    (*info).sneig = calloc(ninterval, sizeof(int));
    (*info).DVtag = calloc(ninterval, sizeof(int));
    ...
    int intervalid, intervalnp;
    color = myid % (*info).ninterval;
    MPI_Comm_split(MPI_COMM_WORLD, color, myid, &intervalcomm);
    MPI_Comm_rank(intervalcomm, &intervalid);
    MPI_Comm_size(intervalcomm, &intervalnp);

    MPI_Comm mumpscomm;
    int mumpsid, mumpsnp;
    color = intervalid % (*info).nGau ;
    MPI_Comm_split(intervalcomm, color, myid, &mumpscomm);
    ...
    MPI_Comm inhostcomm;
    int inhostid, inhostnp;
    MPI_Comm_split(intervalcomm, mumpsid, myid, &inhostcomm);
    MPI_Comm_rank(inhostcomm, &inhostid);
    MPI_Comm_size(inhostcomm, &inhostnp);
    // printf("myid  %d  mumpsid  %d   hostid  %d  hostnp %d\n", myid, mumpsid, hostid, hostnp);

    //printf("myid  %d  mumpsid  %d   inhostid  %d  inhostnp %d\n", myid, mumpsid, inhostid, inhostnp);

    (*info).myid = myid;
    (*info).np = np;

    (*info).intervalcomm = intervalcomm;
    (*info).intervalid = intervalid;
    (*info).intervalnp = intervalnp;
    ...
    (*info).inhostcomm = inhostcomm;
    (*info).inhostid = inhostid;
    (*info).inhostnp = inhostnp;

    if ((*info).myid == 0)
        (*info).interval[0] = mineig;
...
void InitData(SparseMatrix *A, Info *info, char *argv[])
{
    ...
    MPI_Bcast(&N, 1, MPI_INT, 0, (*info).intervalcomm);
...
void InitMatrixLoc(SparseMatrixLoc *A_loc, SparseMatrix A, Info *info)
{
    ...
    MPI_Bcast(infonnz_loc, np, MPI_INT, 0, (*info).intervalcomm);
    ...
    if (myid == 0)
    {
        ...
        {
            mpitag = 1;
            MPI_Send(ia + infofstrow[i], infoN_loc[i] + 1, MPI_INT,    i, mpitag, (*info).intervalcomm);
            mpitag = 2;
            MPI_Send(ja + offset,        infonnz_loc[i],   MPI_INT,    i, mpitag, (*info).intervalcomm);
            mpitag = 3;
            MPI_Send(a + offset,         infonnz_loc[i],   MPI_DOUBLE, i, mpitag, (*info).intervalcomm);
    ...
    else
    {
        ...
        MPI_Status status;
        mpitag = 1;
        MPI_Recv(ia_loc, N_loc + 1, MPI_INT,    0, mpitag, (*info).intervalcomm, &status);
        mpitag = 2;
        MPI_Recv(ja_loc, nnz_loc,   MPI_INT,    0, mpitag, (*info).intervalcomm, &status);
        mpitag = 3;
        MPI_Recv(a_loc,  nnz_loc,   MPI_DOUBLE, 0, mpitag, (*info).intervalcomm, &status);
...
void InitMul(SPMultiply *SPMul, SparseMatrixLoc A_loc, Info *info)
{
    ...
    for (i = 1; i < np; i++)
    {
        ...
        if (myid < np / 2 )
        {
            MPI_Recv(send_count + sourceid, 1, MPI_INT, sourceid, myid, (*info).intervalcomm, &status);
            MPI_Send(recv_count + destid,   1, MPI_INT, destid,   destid, (*info).intervalcomm);
        }
    ...
    while (idsend < np || idrecv < np)
    {
        ...
        if (idsend == idrecv)
        {
            if (idrecv < myid)
            {
                MPI_Recv(send_rowid + recvoffset, send_count[idrecv], MPI_INT, idrecv, myid, (*info).intervalcomm, &status);
                recvoffset = recvoffset + send_count[idrecv];
                idrecv++;
            }
            if (idsend > myid)
            {
                MPI_Send(rowid_out + sendoffset, recv_count[idsend], MPI_INT,   idsend, idsend, (*info).intervalcomm);
...
void InitSystem(LinearSystem *LSystem, SparseMatrixLoc A_loc, Info *info)
{
    ...
    int *C_nnz_loc = calloc((*info).inhostnp, sizeof(int));
    int *C_N_loc = calloc((*info).inhostnp, sizeof(int));
    MPI_Allgather(&mynnz_loc, 1, MPI_INT, C_nnz_loc, 1, MPI_INT, (*info).inhostcomm);
    MPI_Allgather(&myN_loc,   1, MPI_INT, C_N_loc,   1, MPI_INT, (*info).inhostcomm);

    int nnz_loc = 0;
    int N_loc = 0;
    for (i = 0; i < (*info).inhostnp; i++)
    {
    ...
    int *displs = calloc((*info).inhostnp, sizeof(int));
    for (i = 1; i < (*info).inhostnp; i++)
    {
        displs[i] = displs[i - 1] + C_nnz_loc[i - 1];
    }
    MPI_Allgatherv(myic_loc, mynnz_loc, MPI_INT, ic_loc, C_nnz_loc, displs, MPI_INT, (*info).inhostcomm);
    MPI_Allgatherv(myjc_loc, mynnz_loc, MPI_INT, jc_loc, C_nnz_loc, displs, MPI_INT, (*info).inhostcomm);
    MPI_Allgatherv(myc_loc, mynnz_loc, MPI_complex, c_loc, C_nnz_loc, displs, MPI_complex, (*info).inhostcomm);
    ...
    MPI_Allgatherv(myc_diag, myN_loc, MPI_DOUBLE, c_diag, C_N_loc, displs, MPI_DOUBLE, (*info).inhostcomm);
